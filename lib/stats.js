...
module.exports = (function(name, opts) {
    ...
    this.recalculateHistory = function(records) {
        if (records.length > 0) {
            var grouping = 60000,
            ...
            if (Object.keys(bydate).length > 0) {
                ...
                for (date in bydate) {
                    ...
                    recalcRecords.push(row)
                }
                return recalcRecords;
            }
        }
    };
        ...
        _redis.zremrangebyscore(_skey("history"), 0, now - self.historyLimit, function(){
            _redis.zrange(_skey("history"), 0, -1,  function(err, records) {
                _redis.zremrangebyrank(_skey("history"), 0, -1, function() {
                    var rows = self.recalculateHistory(records),
                        redis_addValues = [];

                    for (var i = 0; i < rows.length; i++) {
                        redis_addValues.push([
                                rows[i][STATS_FIELD_CREATED_AT],
                                JSON.stringify(rows[i]),
                                function(redis_key, redis_data, cb){
                                    _redis.zadd(_skey("history"), redis_key, redis_data, cb)
                                }
                        ]);
                    }

                    var callCount = redis_addValues.length;
                    redis_addValues.forEach(function(stats_fn){
                        stats_fn[2].apply(this, [stats_fn[0], stats_fn[1]]);
    ...
    function _groupingByDate(records, grouping) {
        ...
        for (var i = records.length - 1; i >= 0; i--) {
            ...
            if (date in bydate) {
                ...
                bydate[date] = row;
                bydate[date][STATS_FIELD_CREATED_AT] = date;
                counts[date] = 1;
            }

            for (element in bydate) {
                bydate[element][STATS_FIELD_TIME_STARTED] = Math.floor(bydate[element][STATS_FIELD_TIME_STARTED] / counts[element]);
                bydate[element][STATS_FIELD_TIME_PROCESSED] = Math.floor(bydate[element][STATS_FIELD_TIME_PROCESSED] / counts[element]);
                bydate[element][STATS_FIELD_TIME_FINISHED] = Math.floor(bydate[element][STATS_FIELD_TIME_FINISHED] / counts[element]);
            }
        };
