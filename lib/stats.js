...
module.exports = (function(name, opts) {
    ...
    this.normalize = function(opts, fn) {
        ...
        async.parallel([
            ...
            var bydate = _groupingByDate(records, new Date().getTime(), grouping),
                history = [];
    ...
    this.historyReset = function(fn) {
        async.parallel([
            function(cb) { _redis.zremrangebyrank(_skey("history"), 0, -1,  cb) }
        ], fn);
    };
    ...
    function _groupingByDate(records, sdate, grouping) {
        var bydate = {};
        var counts = {};
        for (var i = records.length - 1; i >= 0; i--) {
            var row  = JSON.parse(records[i]);
            if (row[0] < sdate) {
                var date = Math.floor(row[0] / grouping) * grouping;
        ...
        for (elem in bydate) {
            bydate[date][STATS_FIELD_TIME_STARTED] =  Math.floor(bydate[date][STATS_FIELD_TIME_STARTED] / counts[date]);
            bydate[date][STATS_FIELD_TIME_PROCESSED] =  Math.floor(bydate[date][STATS_FIELD_TIME_PROCESSED] / counts[date]);
            bydate[date][STATS_FIELD_TIME_FINISHED] =  Math.floor(bydate[date][STATS_FIELD_TIME_FINISHED] / counts[date]);
        }
    ...
    function _recalculateHistory(fn) {
        async.parallel([
            ...
            var records = replies.shift();

            if (records.length > 0) {
                var grouping = 3600000,
                    bydate = _groupingByDate(records, (new Date().getTime() - (grouping * 24)), grouping);
                ...
                if (Object.keys(bydate).length > 0) {
                    async.parallel([
                        ...
                        for (date in bydate) {
                            ...
                            async.parallel([
                                function(cb) { _redis.zadd(_skey("history"), row[STATS_FIELD_CREATED_AT], JSON.stringify(row), cb) }
                            ], fn);
                        }
                    });
                }
